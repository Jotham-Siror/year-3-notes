...
guided discussion guide and the five concept questions) is **byte-identical across all nine**;
**Part B** has four tasks whose wording is fixed and whose scenario, gas and numbers vary by group.
This file therefore states each stem **once** and tabulates the per-group values.
...
**Common stem, all 9 groups.** Given the scenario's two energy quantities:
...
| **G9** | Wind turbine nacelle | The nacelle **accumulates 1050 kJ** of heat from copper, core and switching losses; the cooling system **removes 870 kJ** to the external airstream over the same period. |
...
> - **G5, G7, G8, G9** give **two heat terms** — heat generated internally and heat removed by a
>   cooling system. **Neither is boundary work.** These are $Q_{in}$ and $Q_{out}$ in a closed system
...
>
> This is a distinction the brief leaves for the group to make; the wording *"dissipates"*,
> *"removes"*, *"rejects … through cooling"* is the tell.

> **Part 3 of the stem is generic across groups** — *"500 identical cycles (e.g., 500 power strokes,
> or 500 charge cycles)"* — and is worded for a reciprocating or charge-cycle system. For the
> continuous-dissipation scenarios (G5, G7, G8, G9) read "cycle" as "repetition of the stated event".

---

## Part B — Task 2: Boundary work at constant pressure ·GA2 Part B Task 2

**Common stem, all 9 groups.** A gas expands at constant pressure from $V_1$ to $V_2$.
...
**Common prompts, all 9 groups.** For the group's scenario:
...
| **G9** | Wind turbines in Kenya's Rift Valley (e.g. Lake Turkana Wind Power) run hot and dusty. Generator output **850 kW** from a mechanical shaft input of **910 kW**; ambient reaches **42 °C**. See the three extra sub-prompts below. |

**G9's Task 4 carries three extra sub-prompts** — it is the only group whose Task 4 is numerical:
...
> **Note on sub-prompt 1.** It asks for a **rate** balance in kW — $\dot{Q}$, $\dot{W}$,
> $\mathrm{d}U/\mathrm{d}t$ — rather than the energy balance in kJ used in Tasks 1–3. FL's balance is
> stated in both forms. → **02 §2.5** → *The four forms of the balance*.
...
Extracted from all **nine** per-group files (`G2_1` … `G2_9`). Part A was diffed across the nine and is
identical apart from the group number; G9's file additionally re-titles its Part B header and expands
Task 4, both recorded above. **No facilitator or model-answer section exists in any of the nine
